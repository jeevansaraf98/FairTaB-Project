...
def calculate_performance_statistical_parity(data, labels, predictions, saIndex, saValue):
    ...
    tpr_non_protected = tp_non_protected / (tp_non_protected + fn_non_protected) if (tp_non_protected + fn_non_protected) != 0 else 0
    tnr_non_protected = tn_non_protected / (tn_non_protected + fp_non_protected) if (tn_non_protected + fp_non_protected) != 0 else 0


    C_prot = (protected_pos) / (protected_pos + protected_neg) if (protected_pos + protected_neg) != 0 else 0
    C_non_prot = (non_protected_pos) / (non_protected_pos + non_protected_neg) if (non_protected_pos + non_protected_neg) != 0 else 0
    ...
    tpr_denom = tp_protected + tp_non_protected + fn_protected + fn_non_protected
    tnr_denom = tn_protected + tn_non_protected + fp_protected + fp_non_protected

    tpr = (tp_protected + tp_non_protected) / tpr_denom if tpr_denom != 0 else 0
    tnr = (tn_protected + tn_non_protected) / tnr_denom if tnr_denom != 0 else 0

    output["balanced_accuracy"] = 0.5 * (tpr + tnr)
    ...
    output["Negative_prot_pred"] = (protected_neg) / (protected_pos + protected_neg) if (protected_pos + protected_neg) != 0 else 0 
    output["Negative_non_prot_pred"] = (non_protected_neg) / (non_protected_pos + non_protected_neg) if (non_protected_pos + non_protected_neg) != 0 else 0 

    return output

#Equalized Odds measure
def calculate_performance_equalized_odds(data, labels, predictions, probs, saIndex, saValue):
    protected_pos = 0.
    protected_neg = 0.
    non_protected_pos = 0.
    non_protected_neg = 0.
...
def calculate_performance_equal_opportunity(data, labels, predictions, saIndex, saValue):
    protected_pos = 0.
    protected_neg = 0.
    non_protected_pos = 0.
    non_protected_neg = 0.
...
#Predictive parity measure
def calculate_performance_predictive_parity(data, labels, predictions, saIndex, saValue):
    protected_pos = 0.
    protected_neg = 0.
    non_protected_pos = 0.
    non_protected_neg = 0.
    ...
    ppv_protected = tp_protected / (tp_protected + fp_protected) if (tp_protected + fp_protected) != 0 else 0
    ppv_non_protected = tp_non_protected / (tp_non_protected + fp_non_protected) if (tp_non_protected + fp_non_protected) != 0 else 0
    ...
    # output["dTPR"] = tpr_non_protected - tpr_protected
    # output["dTNR"] = tnr_non_protected - tnr_protected
    output["fairness"] = abs(ppv_non_protected - ppv_protected)
    # output["fairness"] = abs(stat_par)
...
#Predictive equality measure
def calculate_performance_predictive_equality(data, labels, predictions, saIndex, saValue):
    protected_pos = 0.
    protected_neg = 0.
    non_protected_pos = 0.
    non_protected_neg = 0.
    ...
    fpr_protected = fp_protected / (tn_protected + fp_protected) if (tn_protected + fp_protected) != 0 else 0
    fpr_non_protected = fp_non_protected / (tn_non_protected + fp_non_protected) if (tn_non_protected + fp_non_protected) != 0 else 0
    ...
    # output["dTPR"] = tpr_non_protected - tpr_protected
    # output["dTNR"] = tnr_non_protected - tnr_protected
    output["fairness"] = abs(fpr_protected - fpr_non_protected)
    # output["fairness"] = abs(stat_par)
...
#Treatment equality measure
def calculate_performance_treatment_equality(data, labels, predictions, saIndex, saValue):
    protected_pos = 0.
    protected_neg = 0.
    non_protected_pos = 0.
    non_protected_neg = 0.
    ...
    # output["dTPR"] = tpr_non_protected - tpr_protected
    # output["dTNR"] = tnr_non_protected - tnr_protected
    output["fairness"] = fnpr_protected - fnpr_non_protected
    #if (fnpr_protected!=math.inf) and (fnpr_non_protected!=math.inf):
    #    output["fairness"] = fnpr_protected - fnpr_non_protected
    #else:
    #    output["fairness"] = math.inf
    # output["fairness"] = abs(stat_par)
